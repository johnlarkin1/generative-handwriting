...
model_save_dir = "/Users/johnlarkin/Documents/coding/generative-handwriting/src/saved_models/handwriting_prediction_simple_single_stroke/"
...
if not model_loaded:
    print(
        "No suitable saved model found or model has changed, initializing a new one..."
    )
    stroke_model = DeepHandwritingPredictionModel(
        num_mixture_components=num_mixture_components, stateful=True
    )
...
if last_trained_epoch < desired_epochs or True:
    checkpoint_callback = ModelCheckpoint(
        model_save_path, save_best_only=True, monitor="loss", mode="min"
    )
    stroke_model.fit(
        x_batches,
        y_batches,
...
def plot_predictions_through_sequence(
    ...
    for batch_idx in range(training_data.shape[0]):
        ...
        input_sequence_full = full_reconstructed_data[
            BATCH_OF_INTEREST : BATCH_OF_INTEREST + sequence_length, :
        ]
        ...
        actual_next_point = full_reconstructed_data[
            BATCH_OF_INTEREST + sequence_length, :2
        ]
        x_min, x_max = (
