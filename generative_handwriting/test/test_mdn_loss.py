...
def main():
    ...
    mu1_close = y_true[:, :1] + np.random.normal(loc=0, scale=0.1, size=(50, 1))  # Slightly perturbed x
    mu2_close = y_true[:, 1:2] + np.random.normal(loc=0, scale=0.1, size=(50, 1))  # Slightly perturbed y
    ...
    mu1_far = y_true[:, :1] + np.random.normal(loc=5, scale=2, size=(50, 1))  # Far perturbed x
    mu2_far = y_true[:, 1:2] + np.random.normal(loc=5, scale=2, size=(50, 1))  # Far perturbed y
    ...
    y_actual_far = np.concatenate((pi_far, mu1_far, mu2_far, sigma1_far, sigma2_far, rho_far, eos_far), axis=1)
...
def test_with_two_components():
    ...
    def generate_close_parameters(num_points, num_components):
        pi_components = np.full((num_points, num_components), 0.5)  # Evenly split pi values for simplicity
        mu1 = y_true[:, :1] + np.random.normal(loc=0, scale=0.1, size=(num_points, num_components))
        mu2 = y_true[:, 1:2] + np.random.normal(loc=0, scale=0.1, size=(num_points, num_components))
    ...
    def generate_far_parameters(num_points, num_components):
        pi_components = np.full((num_points, num_components), 0.5)  # Evenly split pi values for simplicity
        mu1 = y_true[:, :1] + np.random.normal(loc=5, scale=2, size=(num_points, num_components))
        mu2 = y_true[:, 1:2] + np.random.normal(loc=5, scale=2, size=(num_points, num_components))
    ...
    pi_far, mu1_far, mu2_far, sigma1_far, sigma2_far, rho_far = generate_far_parameters(num_points, num_components)

    # EOS data - same for both close and far to simplify
    eos = y_true[:, 2:3].reshape(batch_size, num_points, 1)  # Ensure it matches the shape [batch_size, num_points, 1]
    ...
    y_actual_close = np.concatenate([y_actual_close, eos], axis=-1)  # Add EOS at the end

    y_actual_far = np.concatenate([pi_far, mu1_far, mu2_far, sigma1_far, sigma2_far, rho_far], axis=-1).reshape(
    ...
    y_true_reshaped = tf.convert_to_tensor(y_true.reshape([1, num_points, 3]), dtype=tf.float32)

    print(f"For y_actual_close, MDN loss: {mdn_loss(y_true_reshaped, y_actual_close, None, num_components)}")
    print(f"For y_actual_far, MDN loss: {mdn_loss(y_true_reshaped, y_actual_far, None, num_components)}")
