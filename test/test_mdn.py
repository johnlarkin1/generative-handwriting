...
class MDNHeatmapCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        plt.scatter(actual_x_values, actual_y_values, color="blue", label="Actual Sequence Points")
        plt.imshow(
            heatmap,
...
def basic_network():
    ...
    model.compile(optimizer="adam", loss=lambda y_true, y_pred: mdn_loss(y_true, y_pred, num_components=5))
    return model
