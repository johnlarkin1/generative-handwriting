...
def plot_strokes_from_dx_dy(stroke_data, show_image: bool = False):
    """
    ...
    if show_image:
        plt.axis("equal")
        plt.title("Handwriting Stroke Plot")
...
def plot_original_strokes_from_xml(filename, show_image: bool = False) -> None:
    ...
    for stroke in points:
        x, y = zip(*stroke)  # This unzips into x and y coordinates
        plt.plot(x, y)
...
def create_gif(image_folder, matching_source_file_pattern, gif_name, fps: int = 5):
    images = []
    for filename in sorted(glob.glob(f"{image_folder}/{matching_source_file_pattern}.png"), key=os.path.getmtime):
