...
from model.handwriting_prediction import (
    DeepHandwritingPredictionModel,
    SimpleHandwritingPredictionModel,
)
from model.test_lstm import SimpleLSTMModel
...
zigzag_model = SimpleHandwritingPredictionModel(
    units=NUM_LSTM_CELLS_PER_HIDDEN_LAYER, num_layers=NUM_LSTM_HIDDEN_LAYERS, feature_size=3
)
...
zigzag_model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE, clipnorm=ADAM_CLIP_NORM), loss=custom_loss
)
loop_model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE, clipnorm=ADAM_CLIP_NORM), loss=custom_loss
)
...
deep_stroke_model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE, clipnorm=ADAM_CLIP_NORM), loss=model_mdn_loss
)


class SimpleHandwritingVisualizeCallback(Callback):
    def __init__(self, input_sequence, real_sequence, model_name, frequency=10, selected_sequence=60):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch % self.frequency == 0:
            ...
            plt.plot(
                self.real_sequence[0, :, 0], self.real_sequence[0, :, 1], "r-", label="Actual Sequence", linewidth=2
            )
            plt.plot(
                predicted_sequence[0, :, 0], predicted_sequence[0, :, 1], "b--", label="Predicted Sequence", linewidth=2
            )
            plt.scatter(
            ...
            plt.savefig(os.path.join(self.image_dir, f"{self.model_name}_epoch_{epoch}_len{length_of_pred}.png"))
            plt.close()  # Close the figure to free memory


class StrokeVisualizeCallback(Callback):
    def __init__(
        self, input_sequence, real_sequence, reconstructed_data, model_name, frequency=10, selected_sequence=4
    ):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch % self.frequency == 0:
            ...
            y_min, y_max = np.min(self.reconstructed_data[:, 1]), np.max(self.reconstructed_data[:, 1]) + 10
            ...
            pdf_total = mdn_to_heatmap(last_predicted_point, 5, grid_x, grid_y, last_known_point)
...
create_gif(
    "handwriting_visualizations/handwriting_loop_simplified", "handwriting_loop_simplified*", "test_zigzag_simplified"
)
